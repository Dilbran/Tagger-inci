...
GROUP_SUPPORT = os.environ.get("GROUP_SUPPORT", "Hababamsinifi") # Botunuzdan yasaklanan kullanıcıların itiraz işlemleri için başvuracağı grup, kanal veya kullanıcı. Boş bırakırsanız otomatik olarak OWNER_ID kimliğine yönlendirecektir.
...
async def botstats(bot: Client, message: Message):
    ...
    total_users = await db.total_users_count()
    await g4rip.edit(text=LAN.STATS.format(BOT_USERNAME, total_users, groups, pms, total, used, disk_usage, free, cpu_usage, ram_usage, version), parse_mode="md")
...
app.run()
print(">> Bot çalışıyor @nevarevladim  Tarafından Kuruldu<<")
